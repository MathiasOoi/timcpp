...
void del_cons_alloc(std::pair<SVal, SVal>* cons) {cons_alloc.erase(cons);}
...
void del_closure_alloc(Closure* closure) {closure_alloc.erase(closure);}
...
void del_env_alloc(Environment* env) {env_alloc.erase(env);}